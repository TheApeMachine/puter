...
func BenchmarkKernel_RunSwiGLUDTypes(benchmark *testing.B) {
	...
	for _, storageDType := range metalSwiGLUDTypes {
		...
		benchmark.Run(storageDType.Name(), func(benchmark *testing.B) {
			benchmarkSwiGLUDType(benchmark, backend, storageDType)
		})
	}
}

func benchmarkSwiGLUDType(
	benchmark *testing.B,
	backend *Backend,
	...
	for benchmark.Loop() {
		err := runMetalSwiGLU(gate, up, out)
		if err != nil {
			benchmark.Fatal(err)
		}
