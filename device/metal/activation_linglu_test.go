		...
		testingObject.Run(storageDType.Name(), func(testingObject *testing.T) {
			for _, elementCount := range parityElementCounts {
				...
				testingObject.Run(fmt.Sprintf("N=%d", elementCount), func(testingObject *testing.T) {
					convey.Convey(
						"Given Metal "+storageDType.Name()+" tensors for linglu",
						testingObject,
						func() {
							runLinGLUParityCase(testingObject, backend, storageDType, elementCount)
						},
					)
				})
			}
...
func runLinGLUParityCase(
	...
	err := lookupLinGLUKernel(testingObject, storageDType).Run(gate, up, out)
	...
	assertDTypeBytesForTest(testingObject, backend, out, storageDType, fixture.expectedBytes, lingluMaxULP)
}

func lookupLinGLUKernel(testingObject testing.TB, storageDType dtype.DType) kernels.Kernel {
	testingObject.Helper()

	kernel, ok := kernels.Default.LookupLocation("linglu", kernels.Signature{
	...
	if !ok {
		testingObject.Fatalf("linglu Metal kernel not registered for %s", storageDType.Name())
	}
