...
static int metal_layernorm_dispatch_f16(
	MetalDeviceRef contextRef,
	...
	@autoreleasepool {
		...
		id<MTLComputePipelineState> statsPipeline =
			metal_get_pipeline(context, "layernorm_stats_float16", status);
		id<MTLComputePipelineState> applyPipeline =
			metal_get_pipeline(context, "layernorm_apply_float16", status);
...
int metal_dispatch_layernorm_stats(
	...
	@autoreleasepool {
		...
		const char* kernelName = "layernorm_stats_float32";
...
int metal_dispatch_layernorm_apply(
	...
	@autoreleasepool {
		...
		const char* kernelName = "layernorm_apply_float32";
