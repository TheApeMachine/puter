...
func TestLayerNormMetalApplyCPUStatsParity(t *testing.T) {
	harness := metalparity.NewHarness(t)
	defer harness.Close()
	...
	for rowIndex := 0; rowIndex < rows; rowIndex++ {
		...
		variance := float64(cpulayernorm.LayerNormSquaredDiffSumNative(row, wantMean)) / float64(cols)
		wantInvStdDev := float32(1.0 / math.Sqrt(variance+1e-5))
		cpuStats[rowIndex*2] = wantMean
		cpuStats[rowIndex*2+1] = wantInvStdDev
	}
	...
	statsTensor := harness.UploadVector(cpuStats, dtype.Float32)
	defer inputTensor.Close()
