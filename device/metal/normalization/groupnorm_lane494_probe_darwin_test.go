...
func TestGroupNormLane494Probe(t *testing.T) {
	batch := 2
	channels := 8
	groups := 2
	spatial := 64
	...
	spatialIndex := laneIndex % spatial
	...
	t.Logf(
		"lane %d spatialIndex=%d mean=%.9g inv=%.9g effectiveInv=%.9g chain=%.9g fma=%.9g native=%.9g want=%.9g metal=%.9g metalUlp=%d",
		laneIndex,
		spatialIndex,
		mean, invStdDev, effectiveInv,
		chain, fma, nativeOut[laneIndex-groupOffset], want[laneIndex], got[laneIndex],
