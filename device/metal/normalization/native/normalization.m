...
static int metal_groupnorm_dispatch_bf16(
    MetalDeviceRef contextRef,
    MetalBufferRef inputRef,
    MetalBufferRef scaleRef,
    MetalBufferRef biasRef,
    ...
    @autoreleasepool {
        ...
        id<MTLComputePipelineState> statsPipeline =
            metal_get_pipeline(context, "groupnorm_stats_bfloat16", status);
        id<MTLComputePipelineState> applyPipeline =
            metal_get_pipeline(context, "groupnorm_apply_bfloat16", status);
...
int metal_dispatch_groupnorm(
    ...
    if (elementDType == MetalElementDTypeBFloat16) {
        return metal_groupnorm_dispatch_bf16(
            contextRef,
            inputRef,
            scaleRef,
            biasRef,
...
int metal_dispatch_groupnorm_stats(
    ...
    char kernelName[128];
    int nameCode = metal_norm_kernel_name(
        kernelName, sizeof(kernelName), "groupnorm_stats", elementDType, status
    );
...
int metal_dispatch_groupnorm_apply(
    ...
    char kernelName[128];
    int nameCode = metal_norm_kernel_name(
        kernelName, sizeof(kernelName), "groupnorm_apply", elementDType, status
    );
