...
func BenchmarkKernel_RunSiGLUDTypes(benchmark *testing.B) {
	...
	for _, storageDType := range metalSiGLUDTypes {
		...
		benchmark.Run(storageDType.Name(), func(benchmark *testing.B) {
			benchmarkSiGLUDType(benchmark, backend, storageDType)
		})
	}
}

func benchmarkSiGLUDType(
	benchmark *testing.B,
	backend *Backend,
