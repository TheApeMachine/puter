		...
		testingObject.Run(storageDType.Name(), func(testingObject *testing.T) {
			for _, elementCount := range parityElementCounts {
				...
				testingObject.Run(fmt.Sprintf("N=%d", elementCount), func(testingObject *testing.T) {
					convey.Convey(
						"Given Metal "+storageDType.Name()+" tensors for reglu",
						testingObject,
						func() {
							runReGLUParityCase(testingObject, backend, storageDType, elementCount)
						},
					)
				})
			}
...
func runReGLUParityCase(
	...
	err := lookupReGLUKernel(testingObject, storageDType).Run(gate, up, out)
	...
	assertDTypeBytesForTest(testingObject, backend, out, storageDType, fixture.expectedBytes, regluMaxULP)
}

func lookupReGLUKernel(testingObject testing.TB, storageDType dtype.DType) kernels.Kernel {
	testingObject.Helper()

	kernel, ok := kernels.Default.LookupLocation("reglu", kernels.Signature{
	...
	if !ok {
		testingObject.Fatalf("reglu Metal kernel not registered for %s", storageDType.Name())
	}
