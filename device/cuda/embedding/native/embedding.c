...
static const char* g_cuda_embedding_module_source = NULL;

void cuda_embedding_register_module_source(const char* source) {
    g_cuda_embedding_module_source = source;
}

const char* cuda_embedding_module_source(void) {
    return g_cuda_embedding_module_source;
}

void cuda_transformer_status_clear(CUDAStatus* status) {
...
int cuda_transformer_kernel_name(
    ...
    if (suffix == NULL) {
        cuda_transformer_status_set(status, -6, "unknown CUDA embedding dtype");
        return -6;
    }
