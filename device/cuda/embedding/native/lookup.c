...
static int cuda_embedding_dispatch_lookup(
    ...
    void* args[] = {&tablePtr, &indicesPtr, &outPtr, &errorFlagPtr, &vocab, &hidden, &indexCount};
    uint32_t launchCount = indexCount * hidden;
...
int cuda_dispatch_embedding_lookup(
    ...
    return cuda_embedding_dispatch_lookup(
        contextRef,
        "embedding_lookup",
        elementDType,
        tableRef,
