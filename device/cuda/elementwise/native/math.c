...
static const char* cuda_unary_math_operation_name(int operation) {
    switch (operation) {
    case CUDAUnaryMathAbs:
        return "abs";
    case CUDAUnaryMathNeg:
        return "neg";
    case CUDAUnaryMathSqrt:
        return "sqrt";
    case CUDAUnaryMathReLU:
        return "relu";
    default:
        return NULL;
    }
...
int cuda_dispatch_unary_math(
    ...
    const char* operationName = cuda_unary_math_operation_name(operation);

    if (operationName == NULL) {
        cuda_elementwise_status_set(status, -6, "unknown unary math operation");
        return -6;
    }
    ...
    if (dtypeSuffix == NULL) {
        cuda_elementwise_status_set(status, -6, "unknown unary math dtype");
        return -6;
    }
