...
static const char* g_cuda_rope_module_source = NULL;

void cuda_rope_register_module_source(const char* source) {
    g_cuda_rope_module_source = source;
}

const char* cuda_rope_module_source(void) {
    return g_cuda_rope_module_source;
}
...
int cuda_rope_kernel_name(
    ...
    if (suffix == NULL) {
        cuda_rope_status_set(status, -6, "unknown CUDA rope dtype");
        return -6;
    }
