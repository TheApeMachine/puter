...
than returning it (ARCHITECTURE.md §2.2). The CUDA host implementation
still computes on device and reads back internally; once the static
memory planner lands (GAPS.md P1) the host signature will be migrated
to take a device pointer as the output, eliminating the device→host
round-trip entirely.
*/
