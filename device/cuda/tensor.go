...
/*
DeviceTensor is CUDA-resident storage returned from Upload and UploadAsync.
*/
...
func (deviceTensor *DeviceTensor) WaitReady() error {
	...
	code := C.cuda_stream_synchronize(deviceTensor.backend.bridge.uploadStream, &status)
...
func (deviceTensor *DeviceTensor) Close() error {
	...
	if deviceTensor.buffer != nil {
		C.cuda_buffer_release(deviceTensor.buffer)
...
func (deviceTensor *DeviceTensor) bufferRef() C.CUDABufferRef {
