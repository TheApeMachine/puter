...
func runLookupF32Native(
	...
	if cpu.X86.HasAVX2 && cpu.X86.HasFMA {
		runLookupF32AVX2(table, indices, output, vocab, hidden, indexCount)

		return
	}

	if cpu.X86.HasSSE2 {
		runLookupF32SSE2(table, indices, output, vocab, hidden, indexCount)
...
func runBagF32Native(
	...
	if cpu.X86.HasAVX2 && cpu.X86.HasFMA {
		runBagF32AVX2(table, indices, offsets, output, vocab, hidden, bagCount, indexCount)

		return
	}

	if cpu.X86.HasSSE2 {
		runBagF32SSE2(table, indices, offsets, output, vocab, hidden, bagCount, indexCount)
