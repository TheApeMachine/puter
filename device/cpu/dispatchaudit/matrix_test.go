...
func TestBuildCPUDispatchMatrix(t *testing.T) {
	Convey("Given pkg/backend/device/cpu operation domains", t, func() {
		...
		Convey("It should match expected AVX2 and SSE2 registration counts", func() {
			...
			So(counts[ISAPathAVX2], ShouldEqual, 30)
			So(counts[ISAPathSSE2], ShouldEqual, 31)
		})
		...
		Convey("It should register amd64 AVX2 on hot-path domains", func() {
			...
			So(avx2Domains, ShouldResemble, []string{
				"activation", "attention", "causal", "checkpoint", "dequant", "dot", "dropout", "elementwise", "embedding", "hawkes", "interpretability", "layernorm", "losses", "masking", "math",
		...
		Convey("It should register amd64 SSE2 on hot-path domains", func() {
			...
			So(sse2Domains, ShouldResemble, []string{
				"activation", "attention", "causal", "checkpoint", "convolution", "dequant", "dot", "dropout", "elementwise", "embedding", "hawkes", "interpretability", "layernorm", "losses",
				"masking", "math", "matmul", "model_editing", "normalization", "optimizer", "physics", "pool", "pospop", "predictive_coding", "quant", "reduction", "rope", "sampling", "shape", "tokenizer", "vsa",
			})
		})
