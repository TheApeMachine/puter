...
func TestActivationLUTGatherSIMDParity(t *testing.T) {
	convey.Convey("Given f16/bf16 LUT SIMD gather kernels", t, func() {
		lutTables := []struct {
			name string
			lut  *[65536]uint16
		}{
...
func BenchmarkActivationLUTGatherSIMDExp(b *testing.B) {
	count := 8192
	source := make([]uint16, count)
