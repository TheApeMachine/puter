...
func TestActivationReducedLUTParity(t *testing.T) {
	convey.Convey("Given f16/bf16 LUT activation paths", t, func() {
		...
		cases := []lutCase{
			{"Exp", 2, math.FastExp64, activation.Exp},
			{"Log", 2, math.FastLog64, activation.Log},
			{"Log1p", 2, math.FastLog1p64, activation.Log1p},
			{"Expm1", 2, math.FastExpm1_64, activation.Expm1},
			{"Sigmoid", 2, math.FastSigmoid64, activation.Sigmoid},
			{"LogSigmoid", 2, math.FastLogSigmoid64, activation.LogSigmoid},
			{"Tanh", 2, math.FastTanh64, activation.Tanh},
			{"Silu", 2, math.FastSilu64, activation.Silu},
			{"GeluTanh", 2, math.FastGeluTanh64, activation.GeluTanh},
			{"Gelu", 2, math.FastGelu64, activation.Gelu},
			{"ReLU", 1, math.FastReLU64, activation.ReLU},
			{"LeakyReLU", 1, math.FastLeakyReLU64, activation.LeakyReLU},
			{"ELU", 2, math.FastELU64, activation.ELU},
			{"CELU", 2, math.FastCELU64, activation.CELU},
			{"SELU", 2, math.FastSELU64, activation.SELU},
			{"Softplus", 2, math.FastSoftplus64, activation.Softplus},
			{"Mish", 2, math.FastMish64, activation.Mish},
			{"Softsign", 2, math.FastSoftsign64, activation.Softsign},
			{"HardSigmoid", 1, math.FastHardSigmoid64, activation.HardSigmoid},
			{"HardSwish", 1, math.FastHardSwish64, activation.HardSwish},
			{"HardTanh", 1, math.FastHardTanh64, activation.HardTanh},
			{"HardGelu", 1, math.FastHardGelu64, activation.HardGelu},
			{"QuickGelu", 1, math.FastQuickGelu64, activation.QuickGelu},
			{"TanhShrink", 2, math.FastTanhShrink64, activation.TanhShrink},
		}
