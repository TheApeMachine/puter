...
| **f32-only enforcement** | **Dot**, **legacy binary float32 registry**, **interpretability**, **model_editing**, **unary float32 test path** — hard `dtype.Float32` checks |
...
3. **Dot product** (`elementwise_fused_darwin.go`): **`dtype.Float32` only** — gap for f16/bf16 dot on GPU (CPU NEON has them).
...
| `elementwise_fused.metal` | Fused axpy/dot-like | Y | partial | partial | — | Dot dispatch f32-only in Go |
...
| `interpretability.metal` | Activation steer | Y | — | — | — | **f32-only** validate |
| `model_editing.metal` | Weight graft | Y | — | — | — | **f32-only** validate |
...
| **Dot** | `runMetalDot` **f32-only** — need f16/bf16 dot on GPU |
| **Optimality** | Use fused axpy/dot from `elementwise_fused.metal` for all dtypes, not separate slow paths |
...
| Gap | Detail |
|-----|--------|
| **Dot dtypes** | GPU dot **f32-only** in `elementwise_fused_darwin.go` |
...
| **Correctness** | Match CPU optimizer reference at **1 ULP** (CPU NEON Adam currently fails — fix CPU first, then Metal parity) |
...
| Gap | Detail |
|-----|--------|
| **Dtypes** | **`dtype.Float32` only** in validate (`interpretability_darwin.go`, `model_editing_darwin.go`) |
| **Required** | f16/bf16 native steer/graft kernels |
...
3. **Dot / similarity** — f16/bf16 GPU dot (today f32-only in `runMetalDot`).
4. **Interpretability + model_editing** — bf16/fp16 kernels.
